...
def enterTrade(sym, chartDictNew, chartDictOld, session, strategy):
    ...
    tradeDay = session.market_time_manager.getCandleOpenCloseTime(
        chartDictNew["d"][-1].open_ts, "d", n_pre=0, n_post=0
    )["current"]
    intradayCandles = session.getChart(
        [sym], "m1", tradeDay[0].timestamp(), tradeDay[1].timestamp()
    )
    intradayCandles = intradayCandles[sym]
    firstCandleID = 0
    tradeDetailList = strategy.getNewTrade(
        chartDictNew, chartDictOld, intradayCandles, firstCandleID
    )
    for trade in tradeDetailList:
        ...
        if direction == util.TickerStatus.LONG:
            entryID = next(
                (
                    ii
                    for ii, candle in enumerate(intradayCandles)
        ...
        if entryID is None:
            logger.warning(
                f"{sym}: No entry found intraday but expected a {direction.name} entry based on daily chart on {tradeDay[0]}, trigger = {triggerPrice}"
            )
            continue
        ...
        currentTrade = {
            ...
            "direction": direction,
            "exitPrice": exitPrice,
            "exitTimestamp_sec": exitTimestamp,
            "stop type": "",
            "target": targetPrice,
        }
        ...
        for tf in chartDictOld.keys():
            ...
            if len(chartDictNew[tf]) < 3:
                logger.warning(
                    f"Warning: Not enough candles in {tf} timeframe to record combo for {sym} on {tradeDay[0]}. Reduced combo recording."
                )
                currentTrade[tf] = (
                    chartDictNew[tf][-2].to_string()
                    + "-"
                    + currentCandleDict[tf].to_string()
                )
                currentTrade[tf + " combo"] = (
                    chartDictNew[tf][-2].get_kind()
                    + chartDictNew[tf][-2].get_subtype()
                    + "-"
                    + currentCandleDict[tf].get_kind()
                    + currentCandleDict[tf].get_subtype()
                )
            else:
                currentTrade[tf] = (
                    chartDictNew[tf][-3].to_string()
                    + "-"
                    + chartDictNew[tf][-2].to_string()
                    + "-"
                    + currentCandleDict[tf].to_string()
                ...
                currentTrade[tf + " combo"] = (
                    chartDictNew[tf][-3].get_kind()
                    + chartDictNew[tf][-3].get_subtype()
                    + "-"
                    + chartDictNew[tf][-2].get_kind()
                    + chartDictNew[tf][-2].get_subtype()
                    + "-"
        ...
        stopPrice, exit_comment, *targetPrice = strategy.getStop(
            chartDictNew, chartDictOld, currentTrade, intradayCandles, entryID
        )
        currentTrade["stop"] = stopPrice
        currentTrade["initial stop"] = (
            stopPrice  # Initial stop is the same as the stop on the day of entry
        )
        currentTrade["RR"] = (currentTrade["target"] - currentTrade["entryPrice"]) / (
            currentTrade["entryPrice"] - currentTrade["stop"]
        )
        currentTrade["exit_comment"] = exit_comment
        if targetPrice:
            currentTrade["target"] = targetPrice[
                0
            ]  # in case target is changed every day, and not just set at entry
        # Check if stop out the same day (for efficiency, so we don't pull the same 1min data from API again)
        if direction == util.TickerStatus.LONG:
            exitID = next(
                (
                    ii
                    for ii, candle in enumerate(
                        intradayCandles[entryID + 1 :], start=entryID + 1
                    )
                    if (
                        candle.low <= stopPrice or candle.high >= currentTrade["target"]
                    )
                ),
                None,
            )
            ...
            exitID = next(
                (
                    ...
                    if (
                        candle.high >= stopPrice or candle.low <= currentTrade["target"]
                    )
                ),
                None,
            )
...
def updateTrade(trade, chartDictNew, chartDictOld, session, strategy):
    ...
    stopVal, exit_comment, *targetVal = strategy.getStop(
        chartDictNew, chartDictOld, trade
    )
    if targetVal:
        trade["target"] = targetVal[
            0
        ]  # in case target is changed every day, and not just set at entry
    ...
    if (
        trade["direction"] == util.TickerStatus.LONG
        and (
            chartDictNew["d"][-1].low <= trade["stop"]
            or chartDictNew["d"][-1].high >= trade["target"]
        )
    ) or (
        trade["direction"] == util.TickerStatus.SHORT
        and (
            chartDictNew["d"][-1].high >= trade["stop"]
            or chartDictNew["d"][-1].low <= trade["target"]
        )
    ):
        tradeDay = session.market_time_manager.getCandleOpenCloseTime(
            chartDictNew["d"][-1].open_ts, "d", n_pre=0, n_post=0
        ...
        intradayCandles = session.getChart(
            [trade["symbol"]], "m1", tradeDay[0].timestamp(), tradeDay[1].timestamp()
        )
        ...
        if trade["direction"] == util.TickerStatus.LONG:  # Bullish
            if intradayCandles[0].open < trade["stop"]:  # check if we gapped the stop
                ...
                if entryID is None:
                    logger.warning(
                        f"{trade['symbol']}: No {trade['direction'].name} exit found intraday but expected an exit based on daily chart on {tradeDay[0]}, stop = {trade['stop']}, target = {trade['target']}. Keep trade open"
                    )
                    return
...
def printTradeDict(tradesDict, filename):
    ...
    tradesList["entryTimestamp_sec"] = (
        pd.to_datetime(tradesList["entryTimestamp_sec"], unit="s")
        .dt.tz_localize("UTC")
        .dt.tz_convert("America/New_York")
    )
    tradesList["exitTimestamp_sec"] = (
        pd.to_datetime(tradesList["exitTimestamp_sec"], unit="s")
        .dt.tz_localize("UTC")
        .dt.tz_convert("America/New_York")
    )
...
def backtest_symbol(
    dailyChart, chartDict, symbol, market_time_manager, er_list, strategy
):
    session = DataRetrieval(
        rate_limiter=shared_limiter, market_time_manager=market_time_manager
    )  # Recreate session to avoid issues with multiprocessing
    ...
    for day_id in range(2, len(dailyChart)):
        ...
        chartDictNew = addDailyCandleToChart(
            chartDict, lastDay, dayCandleToAdd, dayDateToAdd
        )
        ...
        if (
            pd.to_datetime(chartDictNew["d"][-1].open_ts, unit="s", utc=True).date()
            in er_list
        ):
            for trade in trades:
                if trade["exitPrice"] == -1:
        ...
        chartDict = copy.deepcopy(
            chartDictNew
        )  # Update chartDict to the new one with the latest daily candle
...
def runBacktest(startDay_str, endDay_str, wl, strategy_name, earnings_file):
    ...
    tradeLogFileName = (
        "./Trades/trades_"
        + startDay_str.split(" ")[0].replace("-", "")
        + "_"
        + endDay_str.split(" ")[0].replace("-", "")
        + "_"
        + watchlist_name
        + "_"
        + strategy_name
    ...
    startDay = (
        pd.to_datetime(startDay_str)
        .tz_localize(timezone)
        .replace(hour=6, minute=30, second=0)
    )
    endDay = (
        pd.to_datetime(endDay_str)
        .tz_localize(timezone)
    ...
    startDayToQuery = mtm.getCandleOpenCloseTime(
        startDay.timestamp(), "d", n_pre=2, n_post=0
    )
    ...
    dailyChart = session.getChart(
        symbol_list=watchlist,
        timeframe_sym="d",
        start_timestamp=startDayToQuery["pre"][-1][0].timestamp(),
        end_timestamp=endDay.timestamp(),
    )
    ...
    symbols_missing_daily_candle = [
        symbol
        for symbol in dailyChart.keys()
        if dailyChart[symbol][0].open_ts > startDayToQuery["pre"][-1][0].timestamp()
    ]
    symbol_complete_daily_candle = [
        symbol
        for symbol in dailyChart.keys()
        if dailyChart[symbol][0].open_ts <= startDayToQuery["pre"][-1][0].timestamp()
    ]
    ...
    chartByTimeframe["d"] = {
        symbol: dailyChart[symbol][0:2]
        for symbol in watchlist
        if len(dailyChart[symbol]) > 1
    }  # reminder: [0:2] gets first two candles (inclusive-exclusive range)
    ...
    for TF_sym in TF_sym_list[1:]:
        firstCandleToQuery = mtm.getCandleOpenCloseTime(
            startDay.timestamp(), TF_sym, n_pre=2, n_post=0
        )
        firstCandleToQuery = firstCandleToQuery["pre"][-1]
        chartByTimeframe[TF_sym] = session.getChart(
            symbol_list=symbol_complete_daily_candle,
            timeframe_sym=TF_sym,
            start_timestamp=firstCandleToQuery[0].timestamp(),
            end_timestamp=lastDayBeforeRange[1].timestamp(),
        )
        ...
        for s in symbols_missing_daily_candle:
            if len(dailyChart[s]) < 3:
                logger.warning(
                    f"Not enough daily candles for symbol {s} to form higher timeframe charts. Skipping this symbol."
                )
                continue
            firstDailyCandles = mtm.getCandleOpenCloseTime(
                dailyChart[s][2].open_ts, "d", n_pre=1, n_post=0
            )  # dailyChart[s][2] is the first day to backtest, its open time is startDay

            firstCandleToQuery = mtm.getCandleOpenCloseTime(
                firstDailyCandles["current"][0].timestamp(), TF_sym, n_pre=2, n_post=0
            )  # beginning of two high TF candles prior to first day to backtest
            firstCandleToQuery = firstCandleToQuery["pre"][-1]
            lastDayBeforeBackTest = firstDailyCandles["pre"][
                0
            ]  # last day before backtest day for this symbol
            chart_single_symbol = session.getChart(
                symbol_list=[s],
                timeframe_sym=TF_sym,
                start_timestamp=firstCandleToQuery[0].timestamp(),
                end_timestamp=lastDayBeforeBackTest[1].timestamp(),
            )
    ...
    chartDict = {
        symbol: {
            TF_sym: chartByTimeframe[TF_sym][symbol]
            for TF_sym in chartByTimeframe.keys()
        }
        for symbol in chartByTimeframe[TF_sym].keys()
    }
    ...
    if symbol_invalid:
        logger.warning(
            f"Symbols in daily chart and higher timeframe charts do not match and will be ignored: {symbol_invalid}"
        )
    ...
    with mp.Manager() as manager:
        ...
        with mp.Pool(
            initializer=init_pool, initargs=(limiter, log_queue)
        ) as pool:  # remove number of processes from the call
            total_result = pool.starmap(
                backtest_symbol,
                [
                    (
                        dailyChart[symbol],
    ...
    all_trades = {}
    pnl_by_symbol = {}  # symbol -> total dollar PnL
    capital_by_symbol = {}  # symbol -> total starting value
    num_closed = 0
    num_wins = 0
    for sublist in total_result:
        for trade in sublist:
            symbol = trade["symbol"]
            all_trades.setdefault(symbol, []).append(trade)
            ...
            if trade["direction"] == util.TickerStatus.LONG:
                ...
                pnl = trade["entryPrice"] - trade["exitPrice"]

            qty = trade.get("quantity", 1)
            pnl_dollars = pnl * qty
            start_value = trade["entryPrice"] * qty

            pnl_by_symbol[symbol] = pnl_by_symbol.get(symbol, 0) + pnl_dollars
            capital_by_symbol[symbol] = capital_by_symbol.get(symbol, 0) + start_value
            num_closed += 1
            if pnl_dollars > 0:
                num_wins += 1

    total_pnl = sum(pnl_by_symbol.values())
    total_capital = sum(capital_by_symbol.values())
    total_gain_pct = 100 * total_pnl / total_capital if total_capital else 0
    win_rate = 100 * num_wins / num_closed if num_closed else 0

    # per-symbol summary for DataFrame / CSV
    gain_summary = pd.DataFrame(
        [
            {
                "symbol": s,
                "pnl": pnl_by_symbol.get(s, 0),
                "capital": capital_by_symbol.get(s, 0),
                "gain %": 100 * pnl_by_symbol[s] / capital_by_symbol[s]
                if capital_by_symbol.get(s)
                else 0,
            }
            for s in all_trades
            if s in pnl_by_symbol
        ]
    )
    ...
    ========================
        ...
        Total gain   : {total_gain_pct:.2f}%
        Win rate     : {win_rate:.1f}%  ({num_wins}/{num_closed})
        Closed trades: {num_closed}
...
if __name__ == "__main__":
    ...
    timezone = "America/Los_Angeles"
    earnings_file = "/Users/sepy/dev/stratBot/EarningsCalendar_2025-12-21.csv"
