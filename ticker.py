            ...
            for trade in self.active_trades:
                if trade.check_exit(bar.high, bar.low, update_time.timestamp()):
                    ...
                    log_functions.log_event(
                        ...
                        price=trade.data['exitPrice'],
                        stop_type=trade.data['stop type'],
                        gain_pct=round(trade.data['gain %'], 4),
                        days_open=trade.data.get('daysOpen', 0),
                    )
            for t in closed:
            ...
            if len(chart.get('d', [])) >= 2:
                for s in self.strategies:
                    if s.screenTrade(chart, None):
                        log_functions.log_event("signal", symbol=self.symbol, strategy=s.name)
                        ...
                        for t in trade_list:
                            ...
                            log_functions.log_event(
                                ...
                                price=triggerPrice,
                                stop=round(new_trade.stop, 4),
                                rr=round(new_trade.data['RR'], 2),
                            )
        return
    ...
    def createCandle(self, price, timestamp):
        ...
        for tf in self.TF:
            if self.TF[tf]:
                ...
                candles.pop()
                market_logger.debug(f"{self.symbol}: {tf} candle created @ {price:.2f}")
